...
exports.createBarang = async(req, res) => {
  ...
  `;
        ...
        await logActivity(req.user.id, `Menambahkan barang baru: ${nama_barang} (ID: ${result.insertId})`);
...
exports.deleteBarang = (req, res) => {
    const { id } = req.params;
    db.query('DELETE FROM detail_peminjaman WHERE barang_id = ?', [id], (err, result) => {
        if (err) return res.status(500).json(err);
    });
